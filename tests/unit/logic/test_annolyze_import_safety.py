#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Import-safety tests enforcing the logic/view boundary for AnnoLyze.

The ``app/logic/annolyze`` package must never import tkinter, so it can be
...
LOGIC_DIR = Path(__file__).resolve().parents[3] / "app" / "logic" / "annolyze"
...
def test_logic_package_imports_without_tkinter():
    """Importing app.logic.annolyze must not pull tkinter into sys.modules."""
    for mod in list(sys.modules):
        if mod == "tkinter" or mod.startswith("tkinter."):
            del sys.modules[mod]

    importlib.import_module("app.logic.annolyze")

    assert "tkinter" not in sys.modules, (
        "app.logic.annolyze imported tkinter; logic layer must stay GUI-free."
    )
...
def test_logic_module_has_no_tkinter_import(module_path):
    """No source file under app/logic/annolyze may import tkinter."""
