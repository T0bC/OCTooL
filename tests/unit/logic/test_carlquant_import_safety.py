...
LOGIC_DIR = Path(__file__).resolve().parents[3] / "app" / "logic" / "carlquant"
...
def test_logic_package_imports_without_tkinter():
    """Importing app.logic.carlquant must not pull tkinter into sys.modules."""
    for mod in list(sys.modules):
        if mod == "tkinter" or mod.startswith("tkinter."):
            del sys.modules[mod]

    importlib.import_module("app.logic.carlquant")

    assert "tkinter" not in sys.modules, (
        "app.logic.carlquant imported tkinter; logic layer must stay GUI-free."
    )
...
def test_logic_module_has_no_tkinter_import(module_path):
    """No source file under app/logic/carlquant may import tkinter."""
