...
from app.logic.rexview.models import ExportConfig, SliceExportParams
...
class TestProcessSlice:
    ...
    def test_process_slice_returns_pil_image(
        self, service, sample_3d_image_stack, params, config, metadata
    ):
    ...
    @pytest.mark.unit
    def test_process_slice_applies_refractive_index(
        self, service, sample_3d_image_stack, config, metadata
    ):
...
class TestCalculateDpi:
    ...
    def test_calculate_dpi_all_directions(
        self, service, sample_grayscale_image, metadata, direction
    ):
...
class TestGenerateExportFilename:
    ...
    def test_generate_filename_format(self, service, metadata):
        ...
        filename = service.generate_export_filename(
            params, config, metadata, slice_number=5, export_index=2
        )
...
class TestRunExportPerformance:
    ...
    def _patch_pipeline(self, sample_xml_dict):
        ...
        patchers = [
            ...
            patch(
                "app.logic.rexview.export_service.octF.getXMLAttributes",
                return_value=sample_xml_dict,
            ),
            patch("app.logic.rexview.export_service.octF.createImageFromRaw", return_value=stack),
            patch(
                "app.logic.rexview.export_service.octF.insertScale",
                side_effect=lambda **kw: kw["img"],
            ),
            patch(
                "app.logic.rexview.export_service.octF.createVideoImageFromRaw",
