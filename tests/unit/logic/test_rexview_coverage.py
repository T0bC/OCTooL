...
class TestExportServiceUncovered:
    ...
    def test_run_export_load_progress_callback(self, mock_octF, sample_xml_dict, tmp_path):
        ...
        statuses = []
        service.run_export(
            str(oct_file), params, config, progress_callback=lambda p: statuses.append(p.status)
        )
...
class TestFileDiscoveryUncovered:
    ...
    @pytest.mark.unit
    @patch(
        "app.logic.rexview.file_discovery_service.os.walk", side_effect=PermissionError("denied")
    )
