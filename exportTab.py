...
def addContent(self, frame):
    ...
    self.pickFrame.grid(row=1, column=0, sticky=tk.E + tk.W + tk.N + tk.S)
    
    # SETTINGS
    self.glblSttngsFrame = ttk.LabelFrame(self.exportTabFrame, text='Global Settings', relief=tk.RIDGE)
    self.glblSttngsFrame.grid(row=2, column=0, sticky=tk.E + tk.W + tk.N + tk.S)   
...
# %% Fill Content Frames with widgets here        
    ...
    self.cstmSttngsPanel = customSettings(self.exportTabFrame, self.cstmSttngsFrame, self.treePanel)
    self.pickPanel = pickFile(self.exportTabFrame, self.pickFrame, self.treePanel)
