...
-   **Combined (Recommended)**: A deterministic cascade, not an average of everything. For every A-scan column the software always computes all three depth terms (Half-Span, Knee Point, Inflection); once per slice it then picks which one is trustworthy enough to report, based on how smooth ("laterally stable") each term's depths are across the slice's columns:

    1.  **Half-Span crossing** is tried first and used alone whenever it is stable (lateral SD \<= 12px). It is the primary measure because it is fit-free — unlike a curve fit it cannot fail to converge — and it tracks the visible lesion boundary most closely.
    2.  If Half-Span is unstable, the cascade falls back to the **mean of Knee Point and Inflection**, but only if both of those are themselves stable. The two are biased in opposite, known directions (Knee Point reads deeper than Half-Span, Inflection reads shallower), so averaging them cancels the bias.
    3.  If only one of Knee Point / Inflection is stable, that one is reported alone — biased, but still more informative than no lesion at all.
    4.  If nothing is stable, or the resulting depth is itself laterally noisy (SD \> 15px, a real lesion boundary is smooth), the slice is judged to have no usable lesion signal and the surface line is reported as the depth. This shows as **GATED** in [Validation Mode](#validation-mode).
...
-   **METHOD**: One of Combined, Half-Span, Knee Point, Inflection, Shoulder — Combined is the value actually reported by the app; the rest are its ingredients
...
-   **Half-Span**: Show the half-span crossing depth, the deterministic term the Combined method relies on
...
**Combined Method** (Recommended): A deterministic cascade, not a blend of everything. Half-Span crossing (fit-free, cannot fail to converge) is reported alone whenever it is laterally stable across the slice; otherwise the mean of Knee Point and Inflection is used if both are stable, then whichever of the two is stable alone, and finally the surface line if nothing is stable. See [Method](#method) for the full cascade and [Validation Mode](#validation-mode) for how to score it against operator marks.

**Individual Methods**: - **Half-Span**: Depth where the smoothed intensity profile crosses background + a contrast-adaptive threshold fraction of the span, sustained over several samples. The term Combined leans on; viewable in the A-Scan Viewer but not separately selectable as the primary Method - **Knee Point**: Best for sharp transitions - **Inflection**: Best for smooth S-curves - **Shoulder**: Best for early transition detection

**Method Selection**: The Combined method chooses per slice based on each term's lateral stability (how smoothly its depths hold together across A-scan columns), not on a per-column signal-quality score.
