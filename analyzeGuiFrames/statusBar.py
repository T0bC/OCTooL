...
class StatusBar:
    def __init__(self, context):
        ...
        self.frame = ttk.Frame(self.root, bootstyle="secondary")
        self.frame.grid(row=2, column=0, columnspan=2, sticky="ew")

        self.label = ttk.Label(self.frame, text="Ready", anchor="w", bootstyle="secondary")
        self.label.pack(fill="x", padx=5, pady=2)
        self.label.bind("<Button-1>", self.show_log_window)

        self.context.register_frame("status", self.frame)
        self.context.status_bar = self
    ...
    def update(self, message: str, level: str = "info", duration: int = 2000):
        self.root.after(0, lambda: self._enqueue_message(message, level, duration))
    ...
    def _display_next(self):
        ...
        style, fg = colors.get(level, ("secondary", "black"))
    ...
    def clear(self):
        self.label.config(text="Ready", bootstyle="secondary")
