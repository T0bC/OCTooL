...
class metadataPanel:
    ...
    def setup(self):
        ...
        Tooltip(
            self.operatorEntry,
            text="Define the Operator Abbreviation: TM or CR or ...",
            wraplength=200,
        )
        ...
        Tooltip(
            self.saveConfig,
            text='Save a config file for layout and key bindings. The config is saved as a "json" file and is to be loaded when analysis begins.',
            wraplength=200,
        )

        self.showConfigBtn = ttk.Button(
            self.frame, text="Show Config", command=self.show_keyboard_layout, bootstyle="info"
        )
        self.showConfigBtn.grid(row=2, column=1, sticky="ew", padx=(0, 3), pady=3)
        Tooltip(
            self.showConfigBtn,
            text="View assigned key bindings in a keyboard layout",
            wraplength=200,
        )
