...
class image_viewer_panel(BaseCanvasPanel):
    ...
    def display_image(self, index=None):
        ...
        if not image_list:
            if hasattr(self.context, "status_bar") and self.context.status_bar:
                self.context.status_bar.update(
                    "No images found for selected specimen.", level="warning"
                )
            return
        ...
        if (
            self.last_displayed_slice is not None
            and self.last_displayed_slice != index
            and self.current_slice_modified
        ):
        ...
        try:
            ...
            if hasattr(self.context, "status_bar") and self.context.status_bar:
                self.context.status_bar.update(
                    f"Error displaying image {img_path}: {e}", level="error"
                )
    ...
    def handle_region_click(self, event):
        ...
        if len(self.region_points) < 4:
            ...
            self.save_region_configuration(
                specimen, current_slice, specimen_start, lesion_start, lesion_end, tooth_end
            )
    ...
    def save_region_configuration(
        self, specimen, current_slice, specimen_start, lesion_start, lesion_end, tooth_end
    ):
        """
    ...
    def _interpolate_region_coordinates(self, specimen, total_slices):
        ...
        interpolate_region_coordinates(specimen.config.regions, total_slices, update_region)
    ...
    def start_air_drag(self, event):
        ...
        self.air_drag_start = {"canvas": (start_x, start_y), "image": (image_x, image_y)}
    ...
    def _interpolate_air_coordinates(self, specimen, total_slices):
        ...
        # Use generic interpolation framework
        interpolate_air_coordinates(specimen.config.air, total_slices, update_air)
    ...
    def add_ground_truth_mark(self, event):
        ...
        self.ground_truth_marks.setdefault(slice_index, []).append((float(image_x), float(image_y)))
        ...
        if getattr(self.context, "status_bar", None):
            ...
            self.context.status_bar.update(
                ...
                f"Right-click removes, 'u' undoes, 'c' clears.",
                level="info",
            )
    ...
    def on_validation_right_click(self, event):
        ...
        distances = [((x - image_x) ** 2 + (y - image_y) ** 2) ** 0.5 for x, y in marks]
    ...
    def _draw_ground_truth_curve(self, converter, marks):
        ...
        if len(flattened) >= 4:
            self.canvas.create_line(
                *flattened, fill=GROUND_TRUTH_MARK_COLOR, width=1, tags="ground_truth"
            )
    ...
    def apply_analysis_zoom(self):
        ...
        fitted_zoom = min(canvas_width / self.rawImage.width, canvas_height / self.rawImage.height)
