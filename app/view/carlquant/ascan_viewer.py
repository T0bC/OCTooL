...
class AScanViewer:
    ...
    def show(self):
        ...
        groups = [
            ...
            (
                ...
                [
                    ...
                    ("Ground Truth", self.show_ground_truth, self._on_ground_truth_toggled),
                ],
            ),
            (
                "View",
                [
                    ("Zoom to Analysis", self.zoom_to_analysis, self._on_zoom_to_analysis_toggled),
                ],
            ),
        ]
        ...
        for column, (heading, entries) in enumerate(groups):
            ...
            ttk.Label(toggles_frame, text=heading, font=("Segoe UI", 8, "bold")).grid(
            ...
            for row, (text, variable, command) in enumerate(entries, start=1):
                ttk.Checkbutton(
                    ...
                    text=text,
                    variable=variable,
                    command=command or (lambda: self._update_plot(force_image_sync=True)),
        ...
        # Label above slider
        slider_label = ttk.Label(slider_container, text="A-Scan", font=("Segoe UI", 10, "bold"))
    ...
    def _load_image(self):
        ...
        if self.specimen_id not in specimen_data:
            if hasattr(self.context, "status_bar"):
                self.context.status_bar.update(
                    f"Specimen '{self.specimen_id}' not found.", level="error"
                )
            return False
        ...
        if self.slice_index < 0 or self.slice_index >= len(specimen.images):
            if hasattr(self.context, "status_bar"):
                self.context.status_bar.update(
                    f"Invalid slice index: {self.slice_index}", level="error"
                )
            return False
    ...
    def _plot_ground_truth(self, column_data, surface_y, metadata, lesion_detection_data):
        ...
        if mark is not None:
            ...
            mark_y = val.interpolated_mark_at(self._marks_for_current_slice(), self.current_column)
            if mark_y is None:
                return
        ...
        intensity = column_data[int(mark_y)] if 0 <= int(mark_y) < len(column_data) else 128

        lines = [
            "Ground Truth (interpolated)" if is_interpolated else "Ground Truth (operator)",
            f"X: {intensity:.1f}",
            f"Y: {mark_y:.1f}",
        ]
        ...
        errors = self._method_errors_against(mark_y, surface_y, metadata, lesion_detection_data)
        if errors:
            lines.append("")
    ...
    def _method_errors_against(self, mark_y, surface_y, metadata, lesion_detection_data):
    ...
    def _plot_point_with_hover(
        self, x, y, hover_label, legend_label, marker="o", color="white", markersize=8, zorder=5
    ):
    ...
    def _update_plot(self, force_image_sync=False):
        ...
        if self.slice_result:
            ...
            if hasattr(self.slice_result, "lesion_depth") and self.slice_result.lesion_depth:
                ...
                if (
                    lesion_depth.lesion_detection_data
                    and self.current_column in lesion_depth.lesion_detection_data
                ):
        ...
        if surface_y is not None:
            ...
            if np.any(before_surface):
                self.ax.plot(
                    column_data[before_surface],
                    y_positions[before_surface],
                    color="#00d4ff",
                    linewidth=1.0,
                    label="A-Scan (above surface)",
                    zorder=1,
                )
            ...
            if np.any(fitting_region):
                self.ax.plot(
                    column_data[fitting_region],
                    y_positions[fitting_region],
                    color="#1c6b46",
                    linewidth=1.0,
                    label="A-Scan (fitting region)",
                    zorder=1,
                )
            ...
            if np.any(remaining_region):
                self.ax.plot(
                    column_data[remaining_region],
                    y_positions[remaining_region],
                    color="#0088aa",
                    linewidth=1.0,
                    label="A-Scan (below fitting)",
                    zorder=1,
                )
        else:
            # No surface detected, plot entire A-scan in cyan
            self.ax.plot(
                column_data, y_positions, color="#00d4ff", linewidth=1.0, label="A-Scan", zorder=1
            )
        ...
        if self.show_surface.get():
            if surface_y is not None:
                intensity = (
                    column_data[int(surface_y)] if int(surface_y) < len(column_data) else 128
                )
                self._plot_point_with_hover(
                    intensity,
                    surface_y,
            ...
            if interpolated_surface_y is not None:
                intensity = (
                    column_data[int(interpolated_surface_y)]
                    if int(interpolated_surface_y) < len(column_data)
                    else 128
                )
                self._plot_point_with_hover(
                    intensity,
                    interpolated_surface_y,
        ...
        if lesion_detection_data and surface_y is not None:
            ...
            if self.show_ground_truth.get():
                self._plot_ground_truth(column_data, surface_y, metadata, lesion_detection_data)
        ...
        self.ax.set_title(
            f"A-Scan at Column {self.current_column}", fontsize=10, color="#dcdcdc", pad=8
        )
    ...
    def _plot_half_span_construction(
