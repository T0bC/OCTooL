...
class loadImagePanel:
    ...
    def __init__(self, context):
        ...
        self.selectFolderTooltip = (
            "Choose a folder containing CarlQuant data files (e.g., CSV, JSON, etc.)"
        )
        self.selectFolderBtn = ttk.Button(
            self.frame, text="Select Folder", command=self.selectFolder, bootstyle="primary"
        )
        ...
        self.startAnalyzingBtn = ttk.Button(
            self.frame, text="Start Analyzing", command=self.startAnalyzing, bootstyle="success"
        )
    ...
    def removeSelected(self):
        ...
        if not specimen_panel.selected_rows:
            self.context.status_bar.update(
                "No specimen selected. Please select a row first.", level="warning"
            )
            return
        ...
        if (
            hasattr(self.context, "current_specimen_id")
            and self.context.current_specimen_id in removed_ids
        ):
        ...
        if len(removed_ids) == 1:
            self.context.status_bar.update(
                f"Removed specimen '{removed_ids[0]}' from processing list.", level="success"
            )
        else:
            self.context.status_bar.update(
                f"Removed {len(removed_ids)} specimens from processing list.", level="success"
            )
    ...
    def clear_specimen_coordinates(self):
        ...
        if not specimen.config:
            self.context.status_bar.update(
                f"Specimen '{specimen_id}' has no configuration to clear.", level="warning"
            )
            return
        ...
        if not has_regions and not has_air:
            self.context.status_bar.update(
                f"Specimen '{specimen_id}' has no coordinates to clear.", level="warning"
            )
            return
        ...
        if (
            hasattr(self.context, "current_specimen_id")
            and self.context.current_specimen_id == specimen_id
        ):
            viewer_panel = self.context.get_panel("carl_image")
        ...
        self.context.status_bar.update(
            f"Cleared all coordinates for specimen '{specimen_id}'.", level="success"
        )
    ...
    def startAnalyzing(self):
        ...
        if not hasattr(self.context, "specimen_data") or not self.context.specimen_data:
            self.context.status_bar.update(
                "No specimens loaded. Please select a folder first.", level="warning"
            )
            return
