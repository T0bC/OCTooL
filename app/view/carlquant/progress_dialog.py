...
class ProgressDialog:
    ...
    def _build_ui(self):
        ...
        title_label = ttk.Label(
            main_frame, text="Processing CarlQuant Analysis", font=("TkDefaultFont", 12, "bold")
        )
        ...
        self.overall_label = ttk.Label(overall_frame, text=f"Specimen 0 of {self.total_specimens}")
        self.overall_label.pack(anchor=tk.W)

        self.overall_progress = ttk.Progressbar(
            overall_frame, mode="determinate", maximum=self.total_specimens
        )
        ...
        ttk.Label(mode_frame, text="Processing Mode:", font=("TkDefaultFont", 9, "bold")).pack(
            side=tk.LEFT
        )
        ...
        self.slice_label = ttk.Label(specimen_frame, text="Slice 0 of 0")
        self.slice_label.pack(anchor=tk.W, pady=(5, 0))

        self.slice_progress = ttk.Progressbar(specimen_frame, mode="determinate", maximum=100)
        ...
        self.cancel_button = ttk.Button(
            main_frame, text="Cancel", command=self._on_cancel, bootstyle="danger"
        )
        ...
        with self._lock:
            if not self.cancel_requested:
                ...
                self.status_label.config(
                    text="Cancelling... waiting for current slice(s) to finish", foreground="orange"
                )
