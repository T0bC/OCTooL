...
class resultsPanel:
    ...
    def __init__(self, context):
        ...
        self.highlight_color = (
            ROW_HIGHLIGHT_NAVIGATION_COLOR  # Current highlight color (green or purple)
        )
    ...
    def refresh_validation_scores(self):
        ...
        if not marks_by_slice:
            self._render_validation_message(
                "No marks yet. Press 'h' to hide the overlay, then click the true lesion end."
            )
            return
        ...
        if not per_slice:
            self._render_validation_message(
                "Marked slices have no analysis results yet. Run the analysis to score them."
            )
            return
    ...
    def _update_overlay_hint(self, image_panel):
        ...
        if getattr(image_panel, "overlays_visible", False):
            self.overlayHintLabel.config(
                text="Overlay VISIBLE - press 'h' to hide it before marking", bootstyle="warning"
            )
        else:
            self.overlayHintLabel.config(
        ...
        for method in val.METHODS:
            ...
            rows.append(
                [
                    ...
                    val.format_error(slice_scores.get("abs_median"), signed=False)
                    if has_slice_marks
                    else "--",
                    val.format_error(slice_scores.get("p90"), signed=False)
                    if has_slice_marks
                    else "--",
    ...
    def load_results_for(self, specimen_id: str):
        ...
        if not specimen.results:
            self.context.status_bar.update(
                f"No results available for '{specimen_id}'.", level="warning"
            )
    ...
    def highlight_row(self, row, color=None):
        ...
        # Highlight the row
        self.sheet.highlight_rows([row], bg=highlight_color, fg="#ffffff")
    ...
    def _on_single_click(self, event):
        ...
        if row is not None:
            ...
            if (
                self.active_ascan_viewer
                and self.active_ascan_viewer.dialog
                and self.active_ascan_viewer.dialog.winfo_exists()
            ):
