...
class BaseCanvasPanel:
    ...
    def __init__(self, context, frame_key, canvas_bg="#505050"):
        ...
        self.canvas = tk.Canvas(
            self.frame, width=1024, height=480, highlightthickness=0, bg=canvas_bg
        )
        ...
        self.scale = ttk.Scale(self.frame, from_=1, to=1, orient="horizontal", bootstyle="warning")
        self.scale.set(1)
    ...
    def render_zoomed_image(self):
        ...
        self.canvas.create_image(
            self.image_offset_x, self.image_offset_y, image=self.tk_image, anchor=tk.NW
        )
    ...
    def save_annotated_images(self, output_folder=None, only_modified=True):
        ...
        if self.is_saving:
            if hasattr(self.context, "status_bar") and self.context.status_bar:
                self.context.status_bar.update(
                    "Image saving already in progress...", level="warning"
                )
            return
        ...
        if output_folder is None:
            ...
            if not output_folder:
                if hasattr(self.context, "status_bar") and self.context.status_bar:
                    self.context.status_bar.update(
                        "No image folder set. Cannot save images.", level="error"
                    )
                return
        else:
            ...
            try:
                ...
                if overlay_img:
                    img = Image.alpha_composite(
                        img.convert("RGBA"), overlay_img.convert("RGBA")
                    ).convert("RGB")
                ...
                if task["metadata_text"]:
                    ...
                    draw.rectangle(
                        [x - 5, y - 5, x + text_width + 5, y + text_height + 5], fill=(0, 0, 0, 180)
                    )
                ...
                if (
                    hasattr(self, "root")
                    and hasattr(self.context, "status_bar")
                    and self.context.status_bar
                ):
                    self.root.after(
                        0, lambda msg=error_msg: self.context.status_bar.update(msg, level="error")
                    )
                self.save_queue.task_done()
