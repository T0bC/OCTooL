...
class imagePanel:
    def __init__(self, context):
        ...
        self.scale = ttk.Scale(self.frame, from_=0, to=2, orient="horizontal", bootstyle="warning")
        self.scale.set(1)
    ...
    def dispImageInCanvas(self):
        ...
        if len(self.treeView.getFocus()) != 0:
            ...
            self.scale.bind(
                "<Left>",
                lambda e: [
                    self.scale.set(int(self.scale.get())),
                    self.showImage(int(self.scale.get()), self.rawImage),
                ],
            )
            self.scale.bind(
                "<Right>",
