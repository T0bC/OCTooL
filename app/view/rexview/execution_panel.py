...
class executionPanel:
    def __init__(self, context):
        ...
        self.executeBtn = ttk.Button(
            self.frame, text="RexView!", width=10, command=self.mainRoutine, bootstyle="success"
        )
        ...
        self.button_label.grid(row=0, column=2, sticky=tk.E + tk.W + tk.N + tk.S, pady=3)

        self.breakBtn = ttk.Button(
            self.frame, text="Cancel!", width=10, command=self.breakAll, bootstyle="warning"
        )
        ...
        self.button_label = ttk.Label(self.frame, text="  ")
        self.button_label.grid(row=0, column=4, sticky=tk.E + tk.W + tk.N + tk.S, pady=3)

        self.quitBtn = ttk.Button(
            self.frame, text="Quit!", width=10, command=self.endProgram, bootstyle="danger"
        )
    ...
    def mainRoutine(self):
        ...
        if not tasks:
            self.context.safe_status_update(
                "Export queue is empty. Nothing to export.", level="warning"
            )
            return

        self.context.safe_status_update(
            f"Starting export of {len(tasks)} file(s)...", level="info", duration=3000
        )
