...
class ExportService:
    ...
    def prepare_export(
        ...
        selected_slices = np.linspace(
            params.first_slice - 1, params.last_slice - 1, params.num_slices
        ).astype(int)
        ...
        if params.slice_direction == "XZ":
            ...
            slices_to_load = np.linspace(0, metadata.dim_y - 1, metadata.dim_y).astype(int)

        # Determine data type to use
        if (
            metadata.data_type == "RawSpectraAndProcessedIntensity" and config.prefer_raw
        ) or metadata.data_type == "RawSpectra":
            sel_data_type = "Raw"
        else:
            sel_data_type = "Processed"

        # Create export directory path
        export_dir = params.get_output_path(Path(params.file_path).parent, metadata.exp_number)
