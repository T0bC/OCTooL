...
class SettingsConfig(BaseModel):
    ...
    export_format: Literal[".png", ".tiff"] = Field(
        default=".tiff", description="Output image format"
    )
    averaging: Literal["none", "incoherent", "coherent"] = Field(
        default="coherent", description="A-scan averaging method"
    )
    tukey_window_size: float = Field(
        default=0.9, ge=0.0, le=1.0, description="Tukey window parameter (0=rect, 1=Hann)"
    )
    show_error: bool = Field(
        default=False, description="Show error messages for missing text files"
    )
    ...
    first_slice: int | None = Field(default=None, ge=1, description="First slice to export")
    last_slice: int | None = Field(default=None, ge=1, description="Last slice to export")
    num_equidistant_slices: int = Field(
        default=25, ge=1, description="Number of equidistant slices"
    )
    ...
    dispersion_type: Literal["Quadratic", "None"] = Field(
        default="Quadratic", description="Dispersion compensation type"
    )
    dispersion_coefficient: int = Field(
        default=-100, ge=-100, le=100, description="Dispersion coefficient value"
    )

    # Slice direction
    slice_direction: Literal["XZ", "YZ", "XY"] = Field(
        default="XZ", description="Image slice orientation"
    )

    # Refractive index
    refractive_index: float = Field(
        default=1.0, ge=0.1, le=5.0, description="Refractive index correction factor"
    )
...
class SliceExportParams(BaseModel):
    ...
    dispersion: tuple[str, str] = Field(
        default=("None", "0"), description="Dispersion compensation (type, coefficient)"
    )
...
class QueueItem(BaseModel):
    ...
    slice_direction: Literal["XZ", "YZ", "XY"] = Field(
        default="XZ", description="Image slice direction"
    )
...
class ImageDisplayConfig(BaseModel):
    ...
    data_type: Literal["Processed", "Raw", "RawSpectra", "RawSpectraAndProcessedIntensity"] = Field(
        default="Processed", description="OCT data type"
    )
    averaging: Literal["none", "incoherent", "coherent"] = Field(
        default="coherent", description="A-scan averaging method"
    )
