...
class FileDiscoveryService:
    ...
    def __init__(
        self,
        xml_reader: Callable[[str, str], str] | None = None,
        xml_dict_reader: Callable[[str], dict[str, Any]] | None = None,
    ):
        """
    ...
    def get_default_db_values(self, data_type: str) -> dict[str, int]:
        ...
        """
        return self.DEFAULT_DB_VALUES.get(data_type, self.DEFAULT_DB_VALUES["Raw"])

    def get_dispersion_coefficient(self, serial_number: str | None) -> int:
        """
    ...
    def resolve_sidecar_path(
        self,
        oct_file_path: Path,
        all_oct_files_in_dir: list[Path] | None = None,
    ) -> Path | None:
        """
