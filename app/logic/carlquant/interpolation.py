...
def interpolate_coordinates(
    ...
    if len(keyframes) == 1:
        _propagate_single_keyframe(keyframes[0], config_dict, total_slices, update_func)
        return

    # Multiple keyframes: interpolate between pairs
    _interpolate_between_keyframes(keyframes, config_dict, descriptor, update_func)

    # Backfill: slices before first keyframe
    _backfill_before_first_keyframe(keyframes[0], config_dict, update_func)
...
def _lerp_point(
    start_point: tuple[int, int], end_point: tuple[int, int], t: float
) -> tuple[int, int]:
    """
...
def interpolate_region_coordinates(
    config_dict: dict[int, Any], total_slices: int, update_func: Callable[[int, Any, bool], None]
) -> None:
