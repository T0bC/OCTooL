...
class Surface:
    ...
    ]  # e.g. {"actual_surface": [...], "interpolated_surface": [...]}
    cluster_labels: list[int] | None = None  # Cluster ID for each point (-1 for noise)
...
class LesionDepth:
    depth_points: list[
        tuple[int, int]
    ]  # (x, y) coordinates of lesion bottom (raw detection points)
    mean_depth: float
    median_depth: float
    sd: float
    se: float
...
class AirConfig:
    ...
    point2: tuple[int, int] | None = None  # Second diagonal point (x, y) for rectangular selection
    is_keyframe: bool = False  # True if manually set by user, False if interpolated/propagated
