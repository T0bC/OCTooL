...
from app.logic.carlquant.specimen_model import LesionDepth, RegionStats, Surface
...
def find_surface_peak(
    column: np.ndarray, threshold_idx: int, search_window: int = 250, min_peak_ratio: float = 0.66
) -> int:
...
def fit_lesion_depth_curve_robust(
...
def fit_reference_surface(
    ...
    sound_points = [(x, y) for x, y in surface_points if x < lesion_start_x or x > lesion_end_x]
...
def detect_cavitation(
    ...
    is_cavitated = (
        mean_cavitation_depth > cavitation_threshold and cavitation_ratio >= min_cavitation_ratio
    )
...
def extract_regions(
    ...
    def extract_region_at(center_x: int, region_type: str, region_index: int) -> RegionStats | None:
...
def detect_surface(image: np.ndarray, air_config=None, region_config=None) -> Surface:
    ...
    filtered_points, cluster_labels = cluster_surface_points(
        raw_points, epsilon=17, min_samples=10, min_cluster_size=180
    )
    ...
    if len(filtered_points) > 3:
        ...
        fitted_curves = fit_surface_curve(
            filtered_points, x_start, x_end, curve_name="actual_surface"
        )
...
def fit_exp2_to_profile(
    ...
    try:
        ...
        params = {"a": popt[0], "b": popt[1], "c": popt[2], "d": popt[3], "success": True}
...
def half_span_fraction(span: float) -> float:
    ...
    """
    fraction = (
        HALF_SPAN_BASE_FRACTION
        - HALF_SPAN_CONTRAST_SLOPE * (span - HALF_SPAN_REFERENCE_SPAN) / 100.0
    )
    return float(np.clip(fraction, *HALF_SPAN_FRACTION_LIMITS))


def detect_depth_half_span(
    intensity_profile: np.ndarray, fraction: float | None = None
) -> tuple[float, dict]:
...
def is_method_stable(depth_series: dict, stability_sd: float = METHOD_STABILITY_SD) -> bool:
...
def calculate_lesion_depth(
    ...
    slice_id=None,
    refractive_index: float = TOOTH_REFRACTIVE_INDEX,
) -> LesionDepth | None:
    """
    ...
    method_raw_points = {"knee_point": [], "sigmoid_fit": [], "sigmoid_shoulder": []}
    ...
    for ascan_x in range(start_x, end_x):
        ...
        inflection_depth = (
            sigmoid_meta.get("inflection_depth", np.nan) if sigmoid_meta.get("success") else np.nan
        )
        inflection_idx = (
            sigmoid_meta.get("inflection_idx", -1) if sigmoid_meta.get("success") else -1
        )
        shoulder_depth = (
            sigmoid_meta.get("shoulder_depth", np.nan) if sigmoid_meta.get("success") else np.nan
        )
        ...
        if detection_method == DepthDetectionMethod.KNEE_POINT:
            ...
            fitted_curve = (
                np.array(sigmoid_meta["fitted_curve"]) if "fitted_curve" in sigmoid_meta else None
            )
