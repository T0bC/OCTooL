...
def getXMLAttributes(xmlContent):
    ...
    is3D = all([imgSizemmY is not None, dimY > 1, spacingY > 0])
    ...
    if dataType != "Processed":
        ...
        if rawSpectralFile:
            ...
            Nx = int(rawSpectralFile.get("ScanRegionEnd0")) - int(
                rawSpectralFile.get("ScanRegionStart0")
            )
...
def createImageFromRaw(
    ...
    if selDataType == "Processed":
        ...
        dBstart = (1119000000 - 1105000000) / (80 - 20) * dBmin + (
            1105000000 - ((1119000000 - 1105000000) / (80 - 20) * 20)
        )
        dBend = (1119000000 - 1105000000) / (80 - 20) * dBmax + (
            1105000000 - ((1119000000 - 1105000000) / (80 - 20) * 20)
        )
        ...
        off0 = smooth(offsetErrorData / xmlDict["offsScale"], int(xmlDict["Nline"] / 32) - 1)[
            ..., np.newaxis
        ]
        ...
        dispersionCorrection = np.exp(
            1j * (dispersionCoefficient * np.transpose(chirpData) ** 2 / xmlDict["Nline"])
        )
        ...
        for idx, slice_num in enumerate(spectral_list):
            ...
            spectralData = np.rot90(
                np.fliplr(
                    np.reshape(spectralData, (xmlDict["Napo"] + xmlDict["Nx"], xmlDict["Nline"]))
                )
            )
            ...
            if ENABLE_NOISE_FLOOR:
                ...
                if NOISE_FLOOR_SMOOTHING:
                    ...
                    noise_floor_smoothed = np.convolve(
                        noise_floor_per_ascan,
            ...
            if averaging != "none":
                ...
                cbScan0Av = np.zeros(
                    shape=(height, int(xmlDict["Nx"] / xmlDict["aScanAv"]), xmlDict["aScanAv"]),
                    dtype=np.complex64,
                )
                ...
                if averaging == "incoherent":
                    processed_slice = octToGV(
                        np.mean(abs(cbScan0Av), axis=2), dBmin, dBmax, advancedFilter
                    )
                elif averaging == "coherent":
                    processed_slice = octToGV(
                        np.mean(cbScan0Av, axis=2), dBmin, dBmax, advancedFilter
                    )
            else:
                processed_slice = octToGV(
                    cBScan0[:, range(0, np.shape(cBScan0)[1], xmlDict["aScanAv"])],
                    dBmin,
                    dBmax,
                    advancedFilter,
                )
...
def octToGV_legacy(cBscan, dBmin: int, dBmax: int, advancedFilter: str):
    ...
    if advancedFilter == "selected":
        ...
        for value in range(len(outlierList[0])):
            temp[outlierList[0][value], outlierList[1][value]] = np.median(
                temp[
                    outlierList[0][value] - 1 : outlierList[0][value] + 2,
