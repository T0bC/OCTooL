    ...
    def mainRoutines(self):
        ...
        for item in enumerate(self.treeView.getChildren()):
            ...
            if (self.xmlDict['dataType'] == 'RawSpectraAndProcessedIntensity' and
                ...
                #### To Do ####
                # if direction is any other the the standard XY view, then load all the spectral into one large array
                # we might dont process the single image slices but rather the entire array (might be faster)
                # if we have the 3D array we subset it like we do in the processed folder.
                ...
                ##### To Do #####
                #! so for processed data the XY XZ YZ export schould be working ---> continue
                self.img = octF.createImageFromRaw(xmlDict=self.xmlDict,
                                                   ...
                                                   averaging=self.globalSettingsFrame.averagingMenu.get(),
                                                   spectral=0, # change this to work with image direction, we need to supply the current slice here
                                                   prefRaw='Processed',
                                                   resizeState=self.globalSettingsFrame.getResizeState(),
                                                   tukeySize=float(self.globalSettingsFrame.getTukeyWinSize()),
                                                   advancedFilter=self.globalSettingsFrame.getAdvancedFilter(),
                ...
                for image in enumerate(self.selectedSliceNumber):
                    ...
                    if self.globalSettingsFrame.ScaleBox.state() == ('selected',):
                        #### ToDo ####
                        # change the array indexing so that it matches the correct XY. XZ, ...y selection in treeview.imgSliceDir
